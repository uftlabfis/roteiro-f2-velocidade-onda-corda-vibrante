...
abstract: ""
url: "https://uftlabfis.github.io/roteiro-f2-velocidade-onda-corda-vibrante/"
bibliography: assets/references/references.bib

format:
  html: default
  uftlabfis-typst:
    output-file: velocidade-onda-corda-vibrante.pdf

other-links:
  - text: Roteiro do Experimento
    href: velocidade-onda-corda-vibrante.pdf
...
::: {#fig-modos-naturais}
![Frequências naturais em uma corda vibrante](assets/images/typst/modos-naturais.svg){width="55%"}
...
Por outro lado, se a corda tem massa $m$ e comprimento total $l$ e está sujeita a uma força de tensão $F$, a velocidade $v$ de propagação da onda também pode ser determinada pela relação:
...
Neste experimento, ondas estacionárias são geradas em uma corda esticada,por meio de vibrações de um gerador de ondas senoidais acionado eletricamente. A disposição do aparelho é mostrada na @fig-kit-ondas. A tensão na corda é igual ao peso da massa suspensa sobre a polia.
...
::: {#fig-kit-ondas}
![Aparato experimental](assets/images/equipamento-ondas-pasco.png)
...
== Primeira Parte: Velocidade de onda na corda vibrante
...
+ Com a trena, meça a distância $L$ entre o orifício da haste metálica do gerador de vibrações e a polia. Esse valor correspondará ao comprimento de onda $lambda$ a ser anotado na @tbl-dados-2.
+ Ligue o gerador de ondas senoidais e ajuste a frequência $f$ de tal modo que o padrão de ondas estacionárias corresponda a 1 (um) comprimento de onda $lambda$ (ver @fig-modos-naturais). Note que nessa configuração, $lambda = L$. Anote o valor de $f$ na @tbl-dados-2.
 
+ Utilize a @eq-vel-lambda-f para calcular a velocidade de onda observada ($v_("obs")$).

#info-box([Atenção], [

  #text(size: 9pt)[O *gerador de ondas senoidais* possui pré-ajuste de 1 Hz e ajuste fino de 0,1 Hz para frequência, mostrada no _display_, e controle de amplitude, conforme ilustrado na imagem abaixo.]
...
== Segunda Parte: Velocidade de onda em função da tensão $F$ e da massa específica $mu$

#figure(
  kind: table,
  caption: [Coleta de dados da segunda parte],
)[
  #table(
...
+ Compare os dois valores de velocidade nos items acima. Utilize a @eq-desvio-perc para calcular o erro percentual ($D (%)$) entre os valores obtidos.

$
  D (%) = abs( (v_("obs") - v_("esp")) / (v_("esp"))) times 100 %
$<eq-desvio-perc>
...
#figure(
  ...
  #table(
    columns: (1fr, 1fr, 1fr),
    table.header([$v_("esp")$ (m/s)], [$v_("obs")$ (m/s)], [$D (%)$]),
    [$$]
  )
]
